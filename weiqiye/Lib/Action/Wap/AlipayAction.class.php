...
class AlipayAction extends BaseAction{
	...
	public function pay(){
		...
		switch ($alipayConfig['paytype'])
		{
			...
			case 'weixin': $alipayConfig['paytype'] = 'Weixin';
				break;
			case 'alipay': $alipayConfig['paytype'] = 'Alipay';
				break;
			case 'tenpayComputer': $alipayConfig['paytype'] = 'Tenpaycomputer';
				break;
		}
		...
		}else if($alipayConfig['paytype']=="Weixin"){
			header('Location:/wxpay/index.php?g=Wap&m=' . $alipayConfig['paytype'] . '&a=pay&price=' . $price . '&orderName=' . $orderName . '&single_orderid=' . $orderid . '&showwxpaytitle=1&token=' . $this->token);
		}else {
			header('Location:?g=Wap&m=' . $alipayConfig['paytype'] . '&a=pay&price=' . $price . '&orderName=' . $orderName . '&single_orderid=' . $orderid . '&token=' . $this->token);
		}
	}
	public function setconfig(){
		...
		$alipay_config['cacert']    = getcwd().'\\weiqiye\\Lib\\ORG\\Alipay\\cacert.pem';
