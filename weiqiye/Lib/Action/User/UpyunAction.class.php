	...
	public function upload(){
		...
		if ($this->upload_type=='upyun'){
			...
			if (IS_POST){
				...
				echo '<script>location.href="/echo/index.php?g=User&m=Upyun&a=upload&error='.$return['error'].'&msg='.$return['msg'].'";</script>';
			}else {
				$this->display('local');
			}
		}
		...
		if(!$upload->upload()) {// 上传错误提示错误信息
			...
			if ($thumb==1){
				...
				if(substr($fileName,-4)=='.mp3'){
					$msg=C('site_url').substr($upload->savePath,1).str_replace($fileName,$fileName,$info[0]['savename']);
				}else{
